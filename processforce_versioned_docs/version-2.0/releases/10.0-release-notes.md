...
:::info
The latest Installer is available to download from [this link](/docs/processforce/releases/download/).
:::

## CompuTec ProcessForce 10.0 Release 40

**Release Date: 25 May 2026**

| Issue Type | Components | Release Note |
| --- | --- | --- |
| Fixed | Scheduling | Fixed scheduling behavior for finished goods without routing when using **Backward** scheduling from **SAP Order Recommendation**. Planned dates and times in the **Scheduling Board** and created **Manufacturing Orders** are now calculated correctly based on the recommended end date and scheduling direction. |
| Fixed | Items | Fixed an issue that could cause the error ``The given key was not present in the dictionary`` when opening **Business Partners Relation** from **Item Details** > **Properties**. |
| Fixed | Scheduling | Fixed an issue where the **Scheduling Board** opened from **Order Recommendation** always used **Forward** direction. The scheduling direction now correctly follows the settings configured in **General Settings** > **ProcessForce** > **Bill of Materials and Manufacturing Orders** , including **Backward** default scheduling type and **Use Sch Type in Sch Board** option. |
| Fixed | Time Bookings | Added validation to prevent creating **Time Bookings** for **Backflush resources** through the backend API, ensuring consistent behavior across application flows. |

## CompuTec ProcessForce 10.0 Release 39 HF2

**Release Date: 15 April 2026**

| Issue Type | Components | Release Note |
| --- | --- | --- |
| Fixed | Production Costing | Fixed an issue where rolling back a **Goods Receipt** created from negative inventory could generate an incorrect adjustment **Journal Entry** with doubled price variance values. **Adjustment** entries are now created with the correct amount. |
| Improved | Manufacturing Order | Improved **Manufacturing Order** transaction handling to reduce database locks and deadlocks during processes such as **Journal Entry** creation, **Manufacturing Order Cost** updates, **Time Booking Cost** updates, and dynamic **Receipt Costing**. |
| Improved | Manufacturing Order | Added **Snapshot Isolation** option in **SAP Business One** > **Administration** > **System Initialization** > **General Settings** > **ProcessForce** > **General** to improve system stability and reduce blocking during high-volume operations. [Read more](https://learn.computec.one/docs/processforce/2.0/troubleshooting/performance-issues#snapshot-isolation-in-sap-business-one-sql-server) |

## CompuTec ProcessForce 10.0 Release 39

**Release Date: 19 March 2026**

| Issue Type | Components | Release Note |
| --- | --- | --- |
| Fixed | Manufacturing Order | Fixed an issue where closing a **Manufacturing Order** could trigger a ``Object reference not set to an instance of an object`` error. You can now close MOs without interruptions or system errors. |
| Fixed | Scheduling | Fixed issues in the **Scheduling Board** when opened from **Order Recommendation**, including incorrect levels for multi-level BOMs, wrong scheduling direction, incorrect dates, and inconsistent operation sequencing. Scheduling results are now correct and aligned with planning logic. |
| Improved | Performance | Improved performance of **Manufacturing Order** creation and closure, especially when working from the **Scheduling Board** and **Order Recommendation**. Operations are now significantly faster after upgrade. |
| Fixed | Items | Fixed an issue where changing a **Revision Status** to ``Obsolete`` caused a system query error. **Revision** updates now work correctly without errors. |
| Improved | Architecture | Improved compatibility with SAP Business One by supporting both 5-parameter and 6-parameter versions of ``SBO_SP_TransactionNotification``, including the optional ``user_id`` (integer type). This prevents installation errors and ensures smoother upgrades and plugin installations. |

## CompuTec ProcessForce 10.0 Release 38

**Release Date: 4 March 2026**

| Issue Type | Component/s | Release Note |
| --- | --- | --- |
| Fixed | Manufacturing Order | Fixed an issue where performing a rollback of a **Goods Receipt from Production** could trigger a bin allocation error ``Fully allocate item to bin locations`` even when bin settings were correct. Rollback now completes without errors. |
| Fixed | Web API | Improved validation when linking **Sales Orders** to **Manufacturing Orders** using the ``POST/PATCH`` method. The system now validates item codes and line numbers and returns an error if invalid values are provided, preventing incorrect or incomplete links from being added to the **Manufacturing Order**. |
| Fixed | Production Costing | Fixed an issue where **Time Booking** costs rounded to zero based on company **Amounts** settings caused **Journal Entry** creation to fail. Costs are now rounded correctly and lines resulting in zero value are skipped to prevent accounting errors. |
| Fixed | Production Costing | Fixed an issue where archived costing records were removed during **Roll-over**. Previous costing history is now preserved correctly, and only the oldest records are removed when the maximum history limit is exceeded. |

## CompuTec ProcessForce 10.0 Release 37

**Release Date: 21 January 2026**

| Issue Type | Component/s | Release Note |
| --- | --- | --- |
| Fixed | Scheduling | Fixed an issue where rescheduling an operation with a **Last Operation Finish** advanced relationship caused only one related starting operation to be rescheduled based on selection order when using the **Update** function in **Resource Planning Board**. |
| Fixed | Manufacturing Order | Fixed an issue where the **Price** field was not displayed in the **Close Manufacturing Orders** and **Document Status Change** wizards even when enabled in **Form Settings**. The field is now visible and respects **Manufacturing Order** authorization settings. |
| Fixed | Scheduling | Improved performance when selecting multiple operations using **Select all below** in the **Resource Planning Board**, preventing long application freezes when working with large numbers of operations. |
| Fixed | Bill of Materials | Fixed an issue where operation sequence numbers displayed on the **Scrap** tab differed from those shown on the **Operations** tab in **Manufacturing Orders**, despite correct sequencing in the routing. |

## CompuTec ProcessForce 10.0 Release 36 HF1

**Release Date – 2 January 2026**

| Issue Type | Component/s | Release Note |
| --- | --- | --- |
| Fixed | Manufacturing Order | Fixed an error that occurred when creating a Pick Order from a Manufacturing Order for serial- or batch-managed materials. |

## CompuTec ProcessForce 10.0 Release 36
...
| Bug | Manufacturing Order | Error `Data Table – invalid column Unique Id [66000-135]` occurs on the batch selection screen. |
| Bug | Manufacturing Order | In Pick Receipt, previously used serial numbers reappear in the Serial/Batch selection window instead of only showing available serials. |
| Bug | .netCore | In Pick Receipt, the Bin Location selection loads all bin locations instead of filtering them by the warehouse specified in the Pick Receipt line. |
| Bug | Manufacturing Order | Issue in selection of Sales Order in Manufacturing Order. |
| Bug | Manufacturing Order | Incorrect Sales Order number in Manufacturing Order. |
| Bug | Manufacturing Order | User is not able to perform Rollback. |

## CompuTec ProcessForce 10.0 Release 35

**Release Date – 18 November 2025**

| Issue Type | Component/s | Release Note |
| --- | --- | --- |
| Bug | Manufacturing Order | Error “Data Source - Invalid field value `66000-19`” occurs when selecting a Bin Location in Pick Order. |
...
| Improvement | Architecture, Health-Check | System health check option added (for easier collection data for support purposes). Click [this link](../support-procedure/health-check.md) to find out more | - |

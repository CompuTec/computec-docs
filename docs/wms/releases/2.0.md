...
| Improvement | Client| Pick Receipt - new option has been added to Custom Configuration -> Production -> Pick Receipt - "Do not save every scan to Pick Receipt" | SMAFM-767 |
...
| Improvement | Client | Custom Query Manager now recognizes lower case letters | - |
| Bug | Client | Delivery from Pick List: An error occurs when trying to save a document created for a warehouse without a bin location. The error message: On the "Content" tab, enter an item or items | SIOWT-3757 |
...
| Bug | Client | Pick Order - an error occurred when trying to change the warehouse from which components will be issued. The error message: There is not enough quantity of bins, for item [ITEM] | SPOLC-1840 |
| Bug | Client | Pick List - An error occurred while trying to scan the barcode on the Storage Information screen. The error message: Object reference not set to an instance of an object. | - |
...
| Bug | Client | Pick Order - data was not saved | - |
| Bug | Client | Unnecessary attempts to connect to one of the CT plugins - an error message appears in the log file. The error message: Cannot connect to Serialization plugin: CompuTec AppEngine address is empty | - |
...
| Bug | Client | Interface Design Manager (UI Template) - information in Custom Configuration did not update after changing the template (in CT_WMS_OCCT it changed, but visually in CC it remained the same) | SWMS-3040 |

...
:::info
The release notes for WMS 3.0 are available [here](/docs/appengine/releases/plugins/release-notes/wms-plugin).
:::
...
| --- | --- | --- | --- |
| Improvement | Client | All translation files have been renamed according to IETF language codes. After installing the new WMS Server, the administrator must reselect the language in the WMS Settings → User Settings submenu | - |
...
| Improvement | Client | Scanner Simulator is now compatible with all WMS clients | - |
...
| Improvement | Custom Configuration | Custom Configuration - Interface Designer - added the possibility to assign a template to either an SAP User or a WMS User, which also resolves the issue with missing templates for SSO users | - |
...
| Improvement | Client | Pick Order -  information about the related Manufacturing Order number is added to the remarks created in WMS | SWMS-2607 |
...
| Bug | Client | WMS Client and Server - fixed an issue where data could be lost when saving a transaction if the WMS Server service stopped unexpectedly | - |
...
| Bug | Android Client | GRPO - fixed an issue where items not managed by batches or serial numbers did not open after scanning in the Whse Info screen | - |
...
| Bug | Server | Pick Receipt - fixed an issue where the multiple batch generator did not work with ProcessForce variables from the template | SWMS-3685 |
...
| Improvement | Client | Catch Weight - the quantity screens have been standarized | - |
...
| Bug | Client | SU Dissasembly - The child SU moved into another SU incorrectly had the Item Type IT instead of SU | - |
...
| Improvement | Server | Added Event Trigger for SU Change in CT Labels | - |
...
| Bug | Client | Pick List - doubled quantity required for Pick List in warehouse without bin location, for databases without ProcessForce. | SWITK-198 |
...
| Improvement | Client | New Custom Configuration option (Production, ProcessForce Pick Receipt): Do not save every scan to the Pick Receipt | - |
...
| Improvement | Client | New Custom Configuration option (Production, ProcessForce Pick Receipt): Return to Remarks window after item receipt. | – |
| Bug | Client | Pick Receipt, From all Pick Receipts: an error occurs upon saving a document when using the following Custom Configuration option: Production > ProcessForce Pick Receipt: Enable receipt from production only on data entry level. | - |
...
**Changes and Improvements**
    ...
    - New Custom Configuration option: ProcessForce Pick Order/Receipt PopUP Timer checkpoint

...
| Bug | Server | Delivery - Catch Weight quantities are now calculated correctly | - |
...
| Improvement | Client | Pick List - the performance and speed have been improved | - |
| Improvement | Client | Stock Transfer - Notifications after scanning SU/SSCCs are now more detailed | SWMS-3600 |
| Improvement | Client | Catch Weight - the quantity screens have been standardized | - |
| Improvement | Client | Delivery - The Packing slip is now available | - |
| Bug | Android Client | Android Client - The app no longer stops responding in certain circumstances | SWMS-3500 |
| Bug | Android Client | Android Client - Now compatible with Android 14 or higher | - |
| Bug | Android Client | Fixed an issue where the scanned date range did not match the server's regional settings. A new option has been added to CC → Scanning to override the system date range. | - |
| Bug | Android Client | Android Client - Some buttons were using the language set in Windows Regional Settings instead of the application's settings - this has been corrected. | - |
| Bug | Client | Pick Receipt - The "Do not save every scan to the Pick Receipt" option was not working with SSCC - this issue has been fixed. | SWMS-3500 |
| Bug | Client | Pick Receipt - Expiry Date was missing when creating a Production Receipt in WMS for items managed by UoM - now correctly displayed. | - |
| Bug | Client | Pick Receipt - Batch with UoM - fixed an issue where the quantity disappeared after scanning the Bin Code | SWMS-3549 |
| Bug | Client | Delivery from Pick List - Extra fields were not visible - now displayed correctly | SWMS-3604 |
| Bug | Client | GRPO - The batch number created from the timestamp was incorrectly formatted - this has been fixed | - |
| Bug | Client | SU - Added validation to ensure that either DistNumber or SysNumber is filled out when required | SWMS-3560 |
| Bug | Client | SU Disassembly - The child SU moved into another SU incorrectly had the Item Type IT instead of SU | - |
| Bug | Client | Whse Info - The Batch/Serial Info view had incorrect query - this has been fixed | - |
| Bug | Client | Pick List - Changing the selection of a batch was not possible—this issue has been resolved | - |
| Bug | Client | Goods Return Request - Not all available batches were displayed - now correctly shown | - |

## CompuTec WMS 2.10.22 R7
...
| Improvement | Custom Configuration | Stock Transfer: Added an option in CC to move only the contents of an SU. | - |
| Improvement | Client | Pick Order: After selecting an item, the focus stays on the same position, instead of returning to the top. | SWMS-3531 |
| Improvement | Custom Configuration | SU: added the option in Custom Configuration to hide unnecessary SU fields | - |
| Improvement | Server | Added Terminal ID to the logs. | - |
...
| Bug | Client | Pick Order - SU FIFO/Substitutes screens prevent Batches opening screen after scanning | - |
...
| Improvement | Server | SU Pick and Pack - it is now possible print an SU label | SWMS-3348 |
| Improvement | Client | Delivery - it is now possible to add attachments to the document | - |
| Bug | Client | Pick List - Negative quantities after changing the quantity when selecting the option CC -> Common ->  Define available quantity for Item/Batch in Quantity field | SWMS-3420 |
| Bug | Client | The Item Code is not displayed if diacritics are present | - |
| Bug | Client | GRPO with PO - the document cannot be saved if one of the base document items has been set to Inactive | SWITK-199 |
| Bug | Client | Delivery from Sales Order - Cannot add existing SU <br/><br/> The error message: <blockquote>Scanned SU warehouse does not match selected warehouse</blockquote> | - |
| Bug | Client | Stock Transfer - the previously selected batch is still listed as available | SFOON-244 |

## CompuTec WMS 2.10.22 R3
...
| Improvement | Client | Added drop-down menu for Valid Values ​​for the Android client. | - |
| Improvement | Client | It is now possible to re-use SSCC. | - |
| Improvement | Client | The Storage Info screen has been added and standardized across the entire WMS | - |
| Improvement | Client | It is possible now to decrease the quantity on the Goods Issue | SWMS-2564 |
| Improvement | Client | Return from GRPO - it is now possible to return the entire SU. | SWMS-3270 |
| Improvement | Client | GRPO from AP Reserve Invoice - it is now possible to change the warehouse when using SU | SWMS-3332 |
| Improvement | Client | Stock Transfer - Put Away - SU Code and SSCC are now on separate lines | - |
| Improvement | Custom Configuration | Custom Query Manager now recognizes lower case letters | - |
| Bug | Client | Delivery from Pick List: An error occurs when trying to save a document created for a warehouse without a bin location. <br/><br/>The error message: <blockquote>On the "Content" tab, enter an item or items </blockquote> | SIOWT-3757 |
| Bug | Client | Picks List with assigned batches - the system asks which line to add the scanned batch to, instead of recognizing the scanned line by batch. | SFOON-244 |
...
| Improvement | Client | New Custom Configuration option (Production, CompuTec ProcessForce Pick Receipt): Do not save every scan to the Pick Receipt | - |
...
| Improvement | Client | CompuTec WMS Client for Windows 10 - from version 2.10.22 requires .NET 8 to be installed | - |
...
| Bug | Client | Pick Receipt - If "Automatically receive materials on Pick Receipt Add/Update" was enabled in  CompuTec ProcessForce, then there was no trigger to create a request for Goods Receipt in CT labels. | - |
...
| Bug | Serialization | CompuTec AppEngine Serial Number is not fully displayed when checking Documents in CompuTec AppEngine. | – |
...
**Changes and Improvements**
    ...
    - On the following transactions: Return, Return PO, Delivery, Pick And Pack, xes different that 253
    - Pick Order: CompuTec ProcessForce Pick Order, and Stock Transfer it is now possible to select all Serial Numbers on the list with one button click

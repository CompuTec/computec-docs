...
1. **CompuTec WebUp App** – runs on CompuTec AppEngine 3.0 and provides all customization and automation capabilities.  
...
CompuTec WebUp provides access to nearly all features exposed through the *SAP Business One Web Client Extensibility Framework*, along with **additional capabilities**, such as embedding CompuTec App views directly into SAP Web Client sections.
...
Yes, CompuTec WebUp fully supports both database platforms.
...
See the CompuTec [Documentation Center](/docs/webup/system-requirements) for up-to-date compatibility details.
...
You need to [install](/docs/appengine/administrators-guide/configuration-and-administration/installation) CompuTec AppEngine 3.0 and download the **CompuTec WebUp App** from the CompuTec AppEngine App Store. You will also need to install **CompuTec WebUp Manager** and **CompuTec WebUp Client** for Google Chrome and Microsoft Edge, respectively, from their extension stores. However, at the time of writing, publication of the extensions in the Google Chrome and Microsoft Edge stores is pending.
:::info
Refer to the official [installation guide](/docs/webup/installation-and-activation) for detailed steps.
:::
...
**Does CompuTec WebUp work with Apps developed using the CompuTec AppEngine plugin framework (SAPUI5 UIs)?**  
...
**Microsoft Edge** is also supported, although due to a known issue (reported to Microsoft), the CompuTec WebUp panel may disappear when switching tabs.
...
Users without CompuTec WebUp entitlement will not see any applied customizations.

**Do I need a license to use CompuTec WebUp Manager?**  
No license is required to use **CompuTec WebUp Manager** for configuration.  
However, end users applying the customizations must have valid CompuTec WebUp entitlements.
...
For details, contact your SAP Partner or email [**webup@computec.one**](mailto:webup@computec.one).

**Do I need to be a CompuTec Partner to sell CompuTec WebUp?**  
Yes. You must hold a valid **CompuTec Partnership Agreement**.  
To apply, contact [**partners@computec.one**](mailto:partners@computec.one).

**As a CompuTec Partner, do I need a license for demo or test environments?**  
No. Partners may use CompuTec WebUp **for free** in demo or testing environments.

**As an SAP Partner using SAP Business One internally, do I need to purchase CompuTec WebUp?**  
Yes, for production environments where you use CompuTec WebUp to customize your own Web Client.

**As a customer, do I need to purchase CompuTec WebUp for development or test environments?**  
Yes. All users executing configured automations or customizations must be licensed, regardless of the environment.

**Is CompuTec WebUp licensed per company database?**  

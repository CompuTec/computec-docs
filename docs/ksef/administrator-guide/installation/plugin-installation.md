...
1. Log in to **CompuTec AppEngine Administration Panel**.
    ...
    ![CompuTec AppEngine login screen with Administration selected in the workspace selector and fields for user name and password](media/plugin-install1.png)

2. Go to **Plugins**.

    ![CompuTec AppEngine Administration Panel with Plugins selected in the Configuration menu and the Store tab open](media/ksef-plugin-inst1.png)

3. Navigate to **Store**.

    ![CompuTec AppEngine Plugins page showing the Store tab, plugin type filters, and a search field](media/ksef-plugin-inst2.png)

4. Find the **CompuTec.KSeF** plugin, and click **Get...** next to the plugin name on the list to install the latest plugin version.

    ![CompuTec AppEngine Plugins store showing the CompuTec.KSeF plugin with a Get button](media/ksef-plugin-inst3.png)
...
5. Click **Get & Install**.
    ...
    ![Confirmation dialog listing the CompuTec.KSeF plugins to add to the AppEngine repository with Get and Install and Close buttons](media/ksef-plugin-inst4.png)

6. Click **+ Assign to AppEngine**.

    ![CompuTec.KSeF assignment dialog showing the Assign AppEngine Instances page and the Assign to AppEngine button](media/ksef-plugin-inst5.png)

7. Select **Company** for installation and click **Accept**.

    ![Company selection dialog for assigning the CompuTec.KSeF plugin](media/ksef-plugin-inst6.png)

8. Select **CompuTec AppEngine Instance** for installation and click **Accept**.

    ![CompuTec AppEngine instance selection dialog for installing the CompuTec.KSeF plugin](media/ksef-plugin-inst7.png)

9. Review the installation details and click **Perform Installation**.

    ![Installation details page with the Perform Installation button](media/ksef-plugin-inst8.png)

10. Click **OK** to confirm the plugin installation.

    ![Plugin installation confirmation dialog with an OK button](media/ksef-plugin-inst9.png)

11. You can now track the installation progress. Once the installation is complete, click **Close**.

    ![Plugin installation progress dialog with a Close button](media/ksef-plugin-inst11.png)

12. Click **Yes** to restart the **CompuTec AppEngine**.

    ![Restart confirmation dialog for CompuTec AppEngine with a Yes button](media/ksef-plugin-inst12.png)

13. Once the restart is complete, click **OK**.

14. Done! The plugin is now installed and ready to use.

    ![CompuTec KSeF plugin installation completion screen](media/ksef-plugin-inst14.png)

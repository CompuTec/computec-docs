...
| New | Resources | Added **Resource Efficiency**, allowing scheduling to automatically extend a resource's run time when its effective availability is below 100%. [Read more](/docs/processforce/user-guide/scheduling/resource-efficiency) |
...
| Improved | Quality Control | Added a **General Settings** option to automatically validate or create **Quality Control Tests** when an operation status changes. [Read more](/docs/processforce/user-guide/quality-control/automatic-creation-of-quality-control-documents#validate-or-create-quality-control-tests-on-operation-status-change) |
...
| --- | --- | --- |
| Improvement | Formulas | Include “Contains”, “OR”, and “StartsWith” as part of the formula functionality. |
| Improvement | MRP, MRP 2.5 | Added the ability to define Due Time in Order Recommendations. |
| Bug | Web API | The “With Defaults” option does not work for Orderless Production. |
| Bug | Manufacturing Order | Issues with scrap and coproduct in Orderless Production. |
| Bug | Formulas | Recalculate quantity automatically when UDF values are changed in BOM and MO. |
| Bug | Routings | Invalid records exist in the @CT_PF_RTG3 and @CT_PF_BOM16 tables. |
| Bug | Manufacturing Order, Operations | Operation Relation Map scaling issues – the map does not restore properly. |
| Bug | Manufacturing Order | Updating remarks triggers the error: `“Item - Invalid item [66000-3]. Form Unique Id: 'CT_PF_Details1'.”` |
| Bug | Quality Control | Revision information is missing in the test protocol generated by QualityControlTestProtocolsGeneratorJob. |
| Bug | Gantt Chart | Incorrect start and end date is displayed in Gantt Chart. |
| Bug | Bill of Materials, Manufacturing Order | Distribution Rules are not displayed in BOM and MO. |
| Bug | Manufacturing Order | Users are able to post CompuTec ProcessForce documents with a future date even when “Allow posting future date” is disabled. |
| Bug | Scheduling | The Actual Effort value is incorrect in the Load Report. |
| Bug | Scheduling | Incorrect Resource quantity is displayed on Reschedule. |
| Bug | Resources | Creating a Resource Calendar Template triggers the error: “This entry already exists in the following tables (@CT_PF_CTL1)”. |
| Bug | Bill of Materials, Manufacturing Order | Issues with Advanced Operation Relations in Bill of Materials and Manufacturing Order. |
| Bug | Manufacturing Order | Block posting of CompuTec ProcessForce documents with a future date does not work correctly due to settings being cached. |
| Bug | Manufacturing Order | Issue in selection of Sales Order in Manufacturing Order. |
| Bug | Bin Locations | The system loads all bin locations instead of filtering them based on the warehouse specified in the Pick Receipt line. |
| Bug | Calendars | Loading the Resource Calendar in PFI triggers the error: “invalid column name: HasContinuousCalendar.” |
| Bug | Manufacturing Order | Issues with Serial Numbers in Pick Receipt. |
| Bug | Common Functions | Mass Item Replace triggers the error: “Data Table - exceeds column data size [66000-148].” |
| Bug | Manufacturing Order | Mass Item Replace triggers the error: “Data Table – exceeds column data size [66000-148].” |
| Bug | Items | UDF on Preferred Vendor is not visible. |
| Bug | Manufacturing Order | User is not able to perform Rollback. |
| Bug | Manufacturing Order | Incorrect Sales Order number is displayed in Manufacturing Order. |
| Bug | Common Functions | Mass Item Replace fails with the error: “DataContext save operation failed with the following error.” |
...
| Issue Type | Component/s | Release Note |
| --- | --- | --- |
| Improvement | Manufacturing Order | Added “Copy Planned Qty to Picked Qty” option to the context menu in Pick Order and Pick Receipt screens. |
| Bug | Inventory | An error message is generated by the CT_PF_QtyOnPickCheck view during processing. |
| Bug | Batches | CompuTec ProcessForce displays the batch selection screen even when the batch has already been generated and selected. |
| Bug | Production Costing | In the Inventory Revaluation document created after Roll-over, the New Price is incorrectly rounded to two decimal places. |
| Bug | Batches | Item Description is missing in the Batch Selection screen. |

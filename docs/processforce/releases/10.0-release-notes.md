...
| Bug | Other | Incorrect Printer Set Behavior while inserting new Request into CT Labels, Requester = PF |
...
| Improvement | PDC | Spanish Language support in PDC settings |
...
| Bug | Production Costing | Costing entries were found to be modified automatically following the roll-over process executed by the AE background job. |
...
| Issue Type | Component/s |Release Note | Related Support Ticket(s)\* |
| --- | --- | --- | --- |
| Bug | API, CompuTec CompuTec CompuTec CompuTec AppEngine | Incorrect CompuTec CompuTec CompuTec CompuTec AppEngine job activity checking in PF API. | – |
...
| Issue Type | Component/s |Release Note | Related Support Ticket(s)\* |
| --- | --- | --- | --- |
| Improvement | API, CompuTec CompuTec CompuTec CompuTec AppEngine | It is now possible to run Roll-over from CompuTec CompuTec CompuTec CompuTec AppEngine Roll-up jobs. | – |
...
:::caution
After upgrading to this version, MO Wizard (CompuTec CompuTec CompuTec CompuTec AppEngine plugin) does not work. We are preparing an MO Wizard version compatible with to ProcessForce 10.0 Release 18, and it will be available soon.
:::
...
| Bug | Quality Control | Quality Control Test: when Test Reference is set to 'CONF' through CompuTec CompuTec CompuTec CompuTec AppEngine interface, then the related 'Pass/Fail' field is not filled automatically. | - |
...
| Improvement | MRP 2.5 | MRP CompuTec CompuTec CompuTec CompuTec AppEngine plugin: Item Description and Revision Name columns added to the Recommendation tab | SPROC-12967 |
...
| Bug | Architecture | CompuTec CompuTec CompuTec CompuTec AppEngine version: an error occurs upon queueing several post requests with the same session ID. <br/><br/>Error message: <blockquote>Transaction End No Lock Applied</blockquote> | SPROC-11688 |
...
| Improvement | Maintenance, Web API | The Plant Maintenance function is available from the ProcessForce CompuTec CompuTec CompuTec CompuTec AppEngine plugin level | – |
...
| Bug | Web API | Operation Time Recording Adjustment can not be created in the CompuTec CompuTec CompuTec CompuTec AppEngine plugin version | SPROC-1 |
...
| Bug | API, Web API | An error upon attempts to Set Bill of Materials Advanced Relations through CompuTec CompuTec CompuTec CompuTec AppEngine Odata. | – |
